...
object GlobalAlignment extends AlignmentStrategy {
  def align[B <: BaseLike, C <: Sequence[B, _]](seq1: C, seq2: C, scorer: ScoringMatrix[B]): Alignment = {
    ...
    @tailrec
    def getAlignment(alignmentA: AlignmentSequence = new AlignmentSequence(""), alignmentB: AlignmentSequence = new AlignmentSequence(""), i: Int = seq1.length, j: Int = seq2.length): Alignment = {
      if (i == 0 && j == 0) {
